...
- [x] T001 `src/i18n/ja.ts` に `setup.moveUp` / `setup.moveDown` / `setup.removePlayer` / `molkkout.errorRequiredFields` / `licenses.*`（5キー）を追加
- [x] T002 [P] `src/i18n/en.ts` に同キーを英語訳で追加（Messages 型コンパイルエラーが消えるまで）
- [x] T003 [P] `src/i18n/fi.ts` に同キーをフィンランド語訳で追加（data-model.md の翻訳値テーブル参照）
...
- [x] T004 [US1] `src/components/LicensesScreen.tsx` を `useTranslation()` 移行: `useGame()` + `isJa` フラグ削除、タイトル / 戻るボタン aria-label / 全文表示ボタン / プライバシーポリシー見出し・本文をすべて `t.licenses.*` に置き換える
- [x] T005 [P] [US1] `src/components/MolkkoutSetupScreen.tsx` 60行目のハードコード日本語エラー文字列を `t.molkkout.errorRequiredFields` に変更
- [x] T006 [P] [US1] `src/components/SetupScreen.tsx` の 147 / 155 / 163 行目の aria-label を `t.setup.moveUp` / `t.setup.moveDown` / `t.setup.removePlayer` に変更
...
- [x] T007 [US2] `src/components/ui/ConfirmDialog.tsx` を更新:
...
- [x] T008 [US3] `src/components/SetupScreen.tsx` 104行目の `maxLength={13}` を `maxLength={12}` に変更（1行修正）
...
- [x] T009 [US4] `src/utils/i18n.ts` の三項演算子を `Record<Language, Messages>` ルックアップオブジェクトに変更（research.md パターン参照、`import type { Messages }` 追加）
- [x] T010 [US4] `src/components/ui/NumberInput.tsx` を新規作成: `variant: 'game' | 'molkkout'` prop で PinInput / MolkkoutInput のスタイル差分を制御（data-model.md の差分テーブル参照）
- [x] T011 [P] [US4] `src/components/GameScreen/PinInput.tsx` を `NumberInput variant="game"` の薄いラッパーに変更（T010 完了後）
- [x] T012 [P] [US4] `src/components/MolkkoutScreen/MolkkoutInput.tsx` を `NumberInput variant="molkkout"` の薄いラッパーに変更（T010 完了後）
- [x] T013 [US4] `src/components/SetupScreen.tsx` の `handleMoveUp` / `handleMoveDown` を `handleMove(index: number, direction: -1 | 1)` に統合（T008 完了後、同ファイル）
...
- [x] T014 [US5] `tests/unit/storage.test.ts` を新規作成してテストを記述（赤フェーズ）:
  - バージョン不一致データ → `null` 返却 + localStorage から削除
  - `settings` が文字列のデータ → `null` 返却 + localStorage から削除
  - 不正 JSON → `null` 返却 + localStorage から削除
  - 正常データ → 正しく復元できる
- [x] T015 [US5] `src/utils/storage.ts` に `isValidStoredState` type guard を追加し `loadState` で `as StoredState` キャストを置き換える（research.md パターン参照、T014 テストが緑になるまで）
...
> **NOTE: 既存の実装コードに対するテストカバレッジ追加。テストが失敗する場合は実装のバグを検出したことを意味し、その時点で修正を行う（Constitution II — 主要機能のテスト保護）**

- [x] T016 [P] [US6] `tests/unit/molkkoutReducer.test.ts` を新規作成:
...
- [x] T017 [P] [US6] `tests/unit/share.test.ts` を新規作成:
...
- [x] T018 [P] `npm run build` を実行して TypeScript strict mode でコンパイルエラーがないことを確認
- [x] T019 `npm test` を実行して全テスト通過（27件→40件以上）を確認
- [ ] T020 手動確認（Constitution III 品質ゲート）:
  - Finnish / English 設定でライセンス画面・SetupScreen・MolkkoutSetupScreen のテキストが正しい言語で表示される
  - SetupScreen でプレイヤー名を13文字入力しようとすると12文字で止まる
  - ConfirmDialog: Tab キーがダイアログ外に出ない / Escape でキャンセル / 背景クリックでキャンセル
  - localStorage を `{"version":2,"game":"BAD"}` に改ざんしてリロード → ホーム画面が正常表示される
  - モバイル（iOS / Android）での基本動作確認（Constitution III MUST）
  - オフライン（機内モード）での動作確認（Constitution III MUST）
