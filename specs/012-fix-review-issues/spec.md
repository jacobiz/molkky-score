...
3. **Given** 確認ダイアログが表示された直後, **When** キーボード操作でフォーカスを確認する, **Then** フォーカスはキャンセルボタンにある（FR-005b）
4. **Given** 確認ダイアログが表示されている, **When** Tab キーを押し続ける, **Then** フォーカスはキャンセルボタンと確認ボタンの間のみを循環し、ダイアログ外へ移動しない（FR-005c）
5. **Given** 確認ダイアログが表示されている, **When** Escape キーを押す, **Then** ダイアログが閉じる（キャンセル扱い）（FR-005d）
...
- **FR-005b**: ConfirmDialog が表示された際、フォーカスはキャンセルボタンへ自動的に移動しなければならない（WCAG 2.1 準拠）
- **FR-005c**: ConfirmDialog 表示中、Tab キーによるフォーカス移動はダイアログ内のボタンのみに限定しなければならない（フォーカストラップ）
- **FR-005d**: ConfirmDialog 表示中に Escape キーを押すと、キャンセルボタンと同等の動作でダイアログが閉じなければならない
...
- Q: ConfirmDialog のフォーカス管理（フォーカストラップ実装するか？） → A: フォーカストラップ + ダイアログ表示時にキャンセルボタンへ自動フォーカス（WCAG 2.1 完全対応）
- Q: ConfirmDialog 表示中の Escape キーの扱い → A: Escape キーでキャンセルとして閉じる（WCAG 2.1 推奨）
