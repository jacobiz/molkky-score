...
- [ ] T060 [P] `GameScreen/index.tsx` のレスポンシブ動作を確認する（375px でスコア一覧上・入力下の縦分割、768px 以上で `md:flex-row` による左右並列）
- [ ] T061 [P] 全コンポーネントのモバイル操作を確認する（タップターゲット最小 44×44px、親指届く範囲に主要ボタン配置）
- [ ] T062 PWA オフライン動作を確認する（`npm run build` → `npm run preview` → ネットワーク無効化でもアプリが動作すること）
- [ ] T063 localStorage 永続化の動作確認をする（ゲーム進行中にリロードし全状態が復元されること）
- [ ] T064 [P] TypeScript strict mode コンパイルチェックを実行する（`npx tsc --noEmit` でエラー0件）
- [ ] T065 [P] Constitution 準拠最終確認をする（不必要な抽象化なし・TDD テスト全パス・外部 state ライブラリ未使用・PWA manifest 正常）
- [ ] T066 [P] 実機（または Chrome DevTools モバイルエミュレータ）でパフォーマンスを確認する（SC-001: セットアップ 60 秒以内・SC-002: スコア入力 10 秒/ターン以内・SC-003: ルール判定が入力後即座に反映されること）
